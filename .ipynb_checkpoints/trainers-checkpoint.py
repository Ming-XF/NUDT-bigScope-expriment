...
def sample(model,
    ...
    for k in range(steps):
        ...
        d2inf_pred = torch.zeros((logits.shape[0], 2)).to(seqs.device) + 0.5

        # pluck the logits at the final step and scale by temperature
        logits = logits[:, -1, :] / temperature  # (batch_size, data_size)

        lat_logits, lon_logits = \
            torch.split(logits, (model.lat_size, model.lon_size), dim=-1)
        ...
        ix = torch.cat((lat_ix, lon_ix), dim=-1)
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(config.max_epochs):
            ...
            seqs_init = seqs[:n_plots, :init_seqlen, :2].to(self.device)
