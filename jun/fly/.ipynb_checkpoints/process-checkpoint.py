...
data = pd.read_csv('../fly.csv', sep='\t')
# data['pos_datetime'] = pd.to_datetime(data['pos_datetime'], unit='s')
outdata = []
static = {}
for (index, (mmsi, item)) in enumerate(data.groupby('aircraft_icao')):
    item = item.sort_values(by='pos_datetime')
    
    points = item[['latitude', 'longitude', 'speed', 'track', 'pos_datetime']]
    points = points[points['latitude'] >= lat_min]
    points = points[points['latitude'] <= lat_max]
    points = points[points['longitude'] >= lon_min]
    points = points[points['longitude'] <= lon_max]
    points = points[points['speed'] >= 0]
    points = points[points['speed'] <= 1500]
    points = points[points['track'] >= 0]
    points = points[points['track'] <= 360]
    
    points['latitude'] = (points['latitude'] - lat_min) / (lat_max - lat_min)
    points['longitude'] = (points['longitude'] - lon_min) / (lon_max - lon_min)
    points['speed'] = points['speed'] / 1500
...
fp_train = open('fly_time30_train.pkl', 'wb')
fp_test = open('fly_time30_test.pkl', 'wb')
fp_valid = open('fly_time30_valid.pkl', 'wb')
